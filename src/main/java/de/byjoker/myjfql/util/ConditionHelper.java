...
public class ConditionHelper {
    ...
    public static List<Column> getRequiredColumns(Table table, List<String> argument, Sorter.Type type, String sorter, Sorter.Order order) {
        if (argument.size() == 0) {
            return null;
        }

        try {
            final String[] where = Objects.requireNonNull(MyJFQL.getInstance().getFormatter().formatString(argument))
                    .replace(" or ", " OR ")
                    .split(" OR ");
