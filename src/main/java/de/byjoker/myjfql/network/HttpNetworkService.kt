...
import de.byjoker.myjfql.network.controller.Mapping
import de.byjoker.myjfql.network.controller.QueryController
import de.byjoker.myjfql.network.controller.SessionController
                ...
                val handler = Handler { context ->
                    ...
                    try {
                        when (mapping.access) {
                            ...
                            AccessLevel.SESSION_AND_NO_SESSION -> {
                                method.invoke(controller, request)
                            }
                        }
                    } catch (ex: Exception) {
                        connection.respond(ErrorResponse(ex.message ?: ex.toString()))
                    }
                }
